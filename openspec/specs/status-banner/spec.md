...
1. **优先级 1** — `helperStatus != .installed`：显示 `.warning` 横幅，消息提示 Helper 未安装，按钮引导到设置页
2. **优先级 2** — macOS 14+ 且 `activeMethod == .smJobBless` 且 `helperStatus == .installed`：显示 `.info` 横幅，消息提示有更现代的部署方式可用，按钮引导到设置页（卸载后重新安装）
