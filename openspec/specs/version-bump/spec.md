...
- **WHEN** 用户执行 `./scripts/bump-version.sh 1.5.0`
- **THEN** 脚本继续执行版本更新流程
...
#### Scenario: 参数格式不合法
- **WHEN** 用户执行 `./scripts/bump-version.sh 1.5`（格式不符合 X.Y.Z）
...
#### Scenario: 替换后值可验证
- **WHEN** 脚本执行完毕
- **THEN** `grep MARKETING_VERSION project.pbxproj` 仅返回新版本号，无旧值残留
...
#### Scenario: 正常执行后的输出
- **WHEN** 脚本成功更新版本号
- **THEN** 标准输出包含形如 `Version: 1.4.0 -> 1.5.0` 和 `Build:   14 -> 45` 的摘要行
